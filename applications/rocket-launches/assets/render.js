...
function paint(data, root) {
  root.innerHTML = data.launches.length
    ? data.launches
        .map(
          (l) =>
            `<article class="row entry"><div><p class="eyebrow">${esc(l.date ? stamp(l.date) : t("unknownTime"))}</p><h2>${esc(short(l.name, 100))}</h2><p>${esc(short(l.location, 60))} · ${esc(short(l.status, 40))}</p></div></article>`,
        )
        .join("")
