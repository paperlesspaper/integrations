...
function paint(data, root) {
  root.classList.add("headline");
  root.dataset.total = data.total;
  root.innerHTML =
    (data.failedFeeds ? `<p class="note">${esc(t("partial"))}</p>` : "") +
    (data.items.length
      ? data.items
          .map(
            (i) =>
              `<article class="row entry"><div><p class="eyebrow">${esc(short(i.source, 50))}${i.date ? " · " + esc(stamp(i.date)) : ""}</p><h2>${esc(short(i.title, 180))}</h2></div></article>`,
          )
          .join("")
