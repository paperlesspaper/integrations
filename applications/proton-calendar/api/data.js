...
function normalizeCalendarUrl(value) {
  ...
  try {
    ...
    throw new Error("Enter a valid Proton Calendar sharing URL");
  }
  const hostname = url.hostname.toLowerCase();
  if (url.protocol !== "https:" || url.username || url.password) {
    throw new Error("Proton Calendar links must use HTTPS without embedded credentials");
  }
  if (hostname !== "calendar.proton.me") {
    throw new Error("The calendar URL must be a Proton Calendar sharing link");
  }
  return url;
}
...
async function readLimitedText(response) {
  ...
  if (Number.isFinite(declared) && declared > MAX_BODY_BYTES) {
    throw new Error("Proton Calendar response is too large");
  }
  if (!response.body) {
  ...
  for (let redirect = 0; redirect <= MAX_REDIRECTS; redirect += 1) {
    ...
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      ...
      try {
        ...
        // The redirect target is still validated even if the response body is already closed.
      }
      if (!location || redirect === MAX_REDIRECTS) {
        throw new Error("Proton Calendar returned too many redirects");
      }
      url = normalizeCalendarUrl(new URL(location, url).href);
      continue;
    }
    if (!response.ok) {
      throw new Error(`Proton Calendar request failed with ${response.status}`);
    }
    return readLimitedText(response);
  }
  throw new Error("Proton Calendar could not be loaded");
}
  ...
  return [
    {
      ...
      start: { dateTime: zonedDateTime(rangeStart, "08:00", timeZone), timeZone },
      end: { dateTime: zonedDateTime(rangeStart, "08:45", timeZone), timeZone },
    ...
    },
    {
      ...
      start: {
        ...
        dateTime: zonedDateTime(addDays(rangeStart, 1), "14:00", timeZone),
        timeZone,
      },
    ...
    },
    {
      ...
      start: {
        dateTime: zonedDateTime(addDays(rangeStart, 7), "17:00", timeZone),
        timeZone,
      },
      end: {
        dateTime: zonedDateTime(addDays(rangeStart, 7), "18:00", timeZone),
        timeZone,
      },
      location: "Neighbourhood lab",
    },
  ];
}
...
export default async function handler({ query = {} }) {
  ...
  return {
    sample: false,
    source: "Proton Calendar sharing link",
