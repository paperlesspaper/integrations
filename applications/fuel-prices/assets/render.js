...
function paint(data, root) {
  ...
  app
    .querySelector("header .eyebrow")
    .append(" · " + data.fuelType.toUpperCase());
  root.innerHTML = data.stations.length
    ? data.stations
        .map(
          (s) =>
            `<article class="row entry"><div><h2>${esc(short(s.name, 65))}</h2><p>${esc(short(s.address, 70))} · ${esc(num(s.distance, 1))} km · ${esc(t(s.open ? "open" : "closed"))}</p></div><div class="value">${esc(num(s.price, 3))}<span class="unit">€/l</span></div></article>`,
        )
        .join("")
    : `<p class="hero">${esc(t("empty"))}</p>`;
}
