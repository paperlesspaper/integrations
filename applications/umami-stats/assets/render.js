...
function paint(data, root) {
  root.classList.add("umami");
  const s = data.stats;
  const max = Math.max(1, ...data.series.map((p) => p.y));
  root.innerHTML = `<p class="note">${esc(t("period", { days: data.days }))}</p><div class="metrics">${[
    ["visitors", s.visitors],
    ...
    .map(
      ([role, value]) =>
        `<article class="metric entry"><h2>${esc(t(role))}</h2><div class="value">${esc(num(value, role === "bounceRate" ? 1 : 0))}${role === "bounceRate" ? '<span class="unit">%</span>' : ""}</div></article>`,
    )
    .join(
      "",
    )}</div><p class="note">${esc(t("visitorTrend"))}</p><div class="chart" role="img" aria-label="${esc(t("visitorTrend"))}">${data.series.map((p) => `<div class="bar" style="height:${Math.max(1, Math.round((p.y / max) * 85))}%" title="${esc(p.x)}: ${esc(p.y)}"><span>${data.series.length <= 10 ? esc(num(p.y)) : ""}</span></div>`).join("")}</div>`;
}
