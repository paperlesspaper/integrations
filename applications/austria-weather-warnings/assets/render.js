...
function paint(data, root) {
  ...
  root.innerHTML =
    `<p class="note">${esc(data.municipality)}</p>` +
    (data.warnings.length
      ? data.warnings
          .map(
            (w) =>
              `<article class="row entry warning-level-${w.level}"><div><p class="eyebrow"><span class="warning-label">${esc(Date.parse(w.start) > Date.now() ? t("upcoming") : t("active"))} · ${esc(t("level"))} ${esc(w.level)} / ${esc(messages.levels[w.level])}</span></p><h2>${esc(messages.warningTypes[w.type] || messages.title)}</h2><p>${esc(stamp(w.start))} – ${esc(stamp(w.end))}</p><p>${esc(short(w.text, 240))}</p></div></article>`,
          )
          .join("")
      : `<p class="hero">${esc(t("noWarnings"))}</p>`);
}
