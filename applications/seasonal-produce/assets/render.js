...
function paint(data, root) {
  const month = new Intl.DateTimeFormat(locale, {
    month: "long",
    timeZone: "UTC",
  }).format(new Date(Date.UTC(2026, data.month - 1, 1)));
  const offset = new Date().getUTCDate() % Math.max(1, data.items.length);
  const items = [
    ...data.items.slice(offset),
    ...data.items.slice(0, offset),
  ].slice(0, 12);
  root.dataset.total = data.items.length;
  root.innerHTML = `<p class="eyebrow">${esc(month)}</p><div class="pills">${items.map((p) => `<article class="produce entry" data-category="${esc(p.category)}" data-kind="${esc(p.kind)}"><strong>${esc(p.name)}</strong><small>${esc(t(p.kind))} · ${esc(t(p.category))}</small></article>`).join("")}</div>${data.next.length ? `<p class="note">${esc(t("nextMonth"))}: ${esc(data.next.slice(0, 4).join(", "))}${data.next.length > 4 ? " …" : ""}</p>` : ""}<p class="note">${esc(t("seasonNote"))}</p>`;
  if (!items.length) root.innerHTML = `<p class="hero">${esc(t("empty"))}</p>`;
}
