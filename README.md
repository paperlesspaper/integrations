...
API routes accept both `GET` requests and same-origin JSON `POST` requests. Use `POST` for
passwords, client secrets, access tokens, and private capability URLs so those values do not appear
in query strings or routine URL access logs.

### Shared Calendar Integrations

Google Calendar, Churchdesk, Outlook Calendar, Nextcloud Calendar, Apple Calendar, Proton Calendar,
Webcal & iCalendar, and CalDAV Calendar use the same reusable calendar renderer from
`@paperlesspaper/openintegration`. Provider adapters only fetch and normalize events; agenda, day,
three-day, week, and year layouts, civil date ranges, recurrence handling, timezone display, fitting,
and eInk styling remain shared. Individual providers may offer fewer views when their upstream range
is limited.

Common settings include `view`, `locale`, `timeZone`, `dayRange`, `maxEvents`, `highlightToday`,
`highlightScale`, and `showLocation`. Each integration adds only the connection and presentation
options required by its datasource.

| Integration | Datasource and access model |
| --- | --- |
| [Google Calendar](applications/google-calendar/README.md) | Uses calendar events supplied by the paperlesspaper host and does not collect Google credentials. |
| [Churchdesk](applications/churchdesk/README.md) | Reads a public ChurchDesk rota by URL or UUID; private organization calendars are not supported. |
| [Outlook Calendar](applications/outlook-calendar/README.md) | Reads one Microsoft 365 work or school mailbox through Microsoft Graph `calendarView`, using an Entra application's client-credentials flow and admin-consented `Calendars.ReadBasic.All`. |
| [Nextcloud Calendar](applications/nextcloud-calendar/README.md) | Reads one private calendar through CalDAV using the instance URL, username, and a dedicated app password, with optional calendar-name or same-origin DAV-path selection. |
| [Apple Calendar](applications/apple-calendar/README.md) | Reads an iCloud calendar's public, read-only `webcal://` or HTTPS subscription feed; Apple Account credentials and private CalDAV are not used. |
| [Proton Calendar](applications/proton-calendar/README.md) | Reads a public **Share with anyone** link, which requires a paid Proton plan; Proton credentials and CalDAV are not used. |
| [Webcal & iCalendar](applications/webcal/README.md) | Reads a public, capability-link, or HTTP Basic-authenticated `webcal://`/HTTP(S) iCalendar feed. |
| [CalDAV Calendar](applications/caldav/README.md) | Reads one exact calendar collection URL with a date-bounded, read-only CalDAV `REPORT` using HTTP Basic authentication. |

JSON `POST` keeps connection values out of request URLs, but it is not a secret vault: the trusted
renderer and server adapter still receive those values, and explicit request-body logging or browser
inspection can expose them. Use a trusted HTTPS deployment, disable request-body logging for these
routes, grant read-only access where possible, and revoke or rotate exposed credentials and sharing
links.
...
| ---------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| <img src="applications/austria-weather-warnings/assets/icon.png" alt="" width="200" height="200"> | [Austria Weather Warnings](applications/austria-weather-warnings/README.md) | [run](https://integrations.paperlesspaper.de/austria-weather-warnings/run) | [config](https://integrations.paperlesspaper.de/austria-weather-warnings/config.json) | Current and upcoming official GeoSphere warnings for one Austrian municipality. |
| <img src="applications/ebike-status/assets/icon.png" alt="" width="200" height="200"> | [E-Bike Status](applications/ebike-status/README.md) | [run](https://integrations.paperlesspaper.de/ebike-status/run) | [config](https://integrations.paperlesspaper.de/ebike-status/config.json) | Battery and ride telemetry from Home Assistant, including Specialized Turbo. |
| <img src="applications/air-quality/assets/icon.png" alt="" width="200" height="200"> | [Air Quality](applications/air-quality/README.md) | [run](https://integrations.paperlesspaper.de/air-quality/run) | [config](https://integrations.paperlesspaper.de/air-quality/config.json) | European air quality, particulate matter and six-hour outlook from Open-Meteo/CAMS. |
| <img src="applications/river-levels/assets/icon.png" alt="" width="200" height="200"> | [River Levels](applications/river-levels/README.md) | [run](https://integrations.paperlesspaper.de/river-levels/run) | [config](https://integrations.paperlesspaper.de/river-levels/config.json) | Current water levels at up to three PEGELONLINE stations. |
| <img src="applications/github-releases/assets/icon.png" alt="" width="200" height="200"> | [GitHub Releases](applications/github-releases/README.md) | [run](https://integrations.paperlesspaper.de/github-releases/run) | [config](https://integrations.paperlesspaper.de/github-releases/config.json) | Newest published releases for up to four GitHub repositories. |
| <img src="applications/paperless-ngx-inbox/assets/icon.png" alt="" width="200" height="200"> | [Paperless-ngx Documents](applications/paperless-ngx-inbox/README.md) | [run](https://integrations.paperlesspaper.de/paperless-ngx-inbox/run) | [config](https://integrations.paperlesspaper.de/paperless-ngx-inbox/config.json) | Document counts and recent additions from a Paperless-ngx instance. |
| <img src="applications/fuel-prices/assets/icon.png" alt="" width="200" height="200"> | [Fuel Prices](applications/fuel-prices/README.md) | [run](https://integrations.paperlesspaper.de/fuel-prices/run) | [config](https://integrations.paperlesspaper.de/fuel-prices/config.json) | Nearby fuel prices from Tankerkönig. |
| <img src="applications/home-assistant-sensors/assets/icon.png" alt="" width="200" height="200"> | [Home Assistant Sensors](applications/home-assistant-sensors/README.md) | [run](https://integrations.paperlesspaper.de/home-assistant-sensors/run) | [config](https://integrations.paperlesspaper.de/home-assistant-sensors/config.json) | Selected Home Assistant sensor and binary sensor states. |
| <img src="applications/home-energy/assets/icon.png" alt="" width="200" height="200"> | [Home Energy](applications/home-energy/README.md) | [run](https://integrations.paperlesspaper.de/home-energy/run) | [config](https://integrations.paperlesspaper.de/home-energy/config.json) | Solar, consumption, yield and battery values from Home Assistant. |
| <img src="applications/mealie/assets/icon.png" alt="" width="200" height="200"> | [Mealie Meal Plan](applications/mealie/README.md) | [run](https://integrations.paperlesspaper.de/mealie/run) | [config](https://integrations.paperlesspaper.de/mealie/config.json) | Read-only Mealie meal plans in the shared calendar layouts. |
| <img src="applications/rocket-launches/assets/icon.png" alt="" width="200" height="200"> | [Rocket Launches](applications/rocket-launches/README.md) | [run](https://integrations.paperlesspaper.de/rocket-launches/run) | [config](https://integrations.paperlesspaper.de/rocket-launches/config.json) | Upcoming launches and schedule status from Launch Library 2. |
| <img src="applications/rss-news/assets/icon.png" alt="" width="200" height="200"> | [RSS News](applications/rss-news/README.md) | [run](https://integrations.paperlesspaper.de/rss-news/run) | [config](https://integrations.paperlesspaper.de/rss-news/config.json) | Headlines from up to four RSS or Atom feeds. |
| <img src="applications/school-timetable/assets/icon.png" alt="" width="200" height="200"> | [School Timetable](applications/school-timetable/README.md) | [run](https://integrations.paperlesspaper.de/school-timetable/run) | [config](https://integrations.paperlesspaper.de/school-timetable/config.json) | Manually maintained lessons in the shared calendar layouts. |
| <img src="applications/seasonal-produce/assets/icon.png" alt="" width="200" height="200"> | [Seasonal Produce](applications/seasonal-produce/README.md) | [run](https://integrations.paperlesspaper.de/seasonal-produce/run) | [config](https://integrations.paperlesspaper.de/seasonal-produce/config.json) | Seasonal German produce, with fresh harvest and storage distinguished. |
| <img src="applications/umami-stats/assets/icon.png" alt="" width="200" height="200"> | [Umami Stats](applications/umami-stats/README.md) | [run](https://integrations.paperlesspaper.de/umami-stats/run) | [config](https://integrations.paperlesspaper.de/umami-stats/config.json) | Visitors, pageviews, visits and a daily visitors chart from Umami. |
...
| <img src="applications/apple-calendar/assets/icon.png" alt="" width="200" height="200">                 | [Apple Calendar](applications/apple-calendar/README.md)                             | [run](https://integrations.paperlesspaper.de/apple-calendar/run)              | [config](https://integrations.paperlesspaper.de/apple-calendar/config.json)              | Shows events from a public, read-only iCloud Calendar subscription.                                                                            |
...
| <img src="applications/caldav/assets/icon.png" alt="" width="200" height="200">                           | [CalDAV Calendar](applications/caldav/README.md)                                    | [run](https://integrations.paperlesspaper.de/caldav/run)                      | [config](https://integrations.paperlesspaper.de/caldav/config.json)                      | Shows one calendar collection through authenticated, read-only CalDAV requests.                                                               |
...
| <img src="applications/churchdesk/assets/icon.png" alt="" width="200" height="200">                        | [Churchdesk](applications/churchdesk/README.md)                                     | [run](https://integrations.paperlesspaper.de/churchdesk/run)                  | [config](https://integrations.paperlesspaper.de/churchdesk/config.json)                  | Shows services and rota assignments from a public ChurchDesk rota.                                                                             |
...
| <img src="applications/human-rights/assets/icon.png" alt="" width="200" height="200">                     | [Human Rights — Article of the Day](applications/human-rights/README.md)             | [run](https://integrations.paperlesspaper.de/human-rights/run)                | [config](https://integrations.paperlesspaper.de/human-rights/config.json)                | Shows one deterministic daily article from the Universal Declaration of Human Rights in English or German, with optional article pinning.     |
...
| <img src="applications/nextcloud-calendar/assets/icon.png" alt="" width="200" height="200">             | [Nextcloud Calendar](applications/nextcloud-calendar/README.md)                     | [run](https://integrations.paperlesspaper.de/nextcloud-calendar/run)          | [config](https://integrations.paperlesspaper.de/nextcloud-calendar/config.json)          | Shows events from a private Nextcloud calendar through read-only CalDAV requests.                                                              |
...
| <img src="applications/outlook-calendar/assets/icon.png" alt="" width="200" height="200">               | [Outlook Calendar](applications/outlook-calendar/README.md)                         | [run](https://integrations.paperlesspaper.de/outlook-calendar/run)            | [config](https://integrations.paperlesspaper.de/outlook-calendar/config.json)            | Shows one Microsoft 365 work or school calendar through Microsoft Graph.                                                                        |
| <img src="applications/proton-calendar/assets/icon.png" alt="" width="200" height="200">                | [Proton Calendar](applications/proton-calendar/README.md)                           | [run](https://integrations.paperlesspaper.de/proton-calendar/run)             | [config](https://integrations.paperlesspaper.de/proton-calendar/config.json)             | Shows events from a read-only Proton Calendar sharing link.                                                                                     |
...
| <img src="applications/webcal/assets/icon.png" alt="" width="200" height="200">                           | [Webcal & iCalendar](applications/webcal/README.md)                                 | [run](https://integrations.paperlesspaper.de/webcal/run)                      | [config](https://integrations.paperlesspaper.de/webcal/config.json)                      | Shows events from a Webcal subscription or an HTTP(S) iCalendar feed URL.                                                                      |
...
The new daily-use integrations cover fuel prices, seasonal produce, school timetables,
Mealie meal plans, RSS news, rocket launches, Umami analytics, Austrian weather warnings,
Home Assistant sensors, home energy and e-bike status. Additional integrations cover
air quality, river levels, GitHub releases and Paperless-ngx documents. They use purposeful Spectra 6
color accents, German/English UI, and both display orientations.

See [setup, network configuration and validation](docs/dashboard-integrations.md),
[guideline feedback](docs/integration-guideline-feedback.md), and
[icon generation prompts](docs/integration-icon-prompts.md). The existing Day Calendar's
`season-daylight` mode now adds a seven-day daylight delta chart using SunCalc.
...
Some integrations call upstream APIs from `api/data.js`. Keep credentials in environment variables
or user-provided settings, never in committed files or screenshot variants. Prefer same-origin JSON
`POST` for user-provided secrets and capability links, use HTTPS, and avoid request-body logging.
`.env`, `node_modules`, generated output, and local workspace files are intentionally ignored.
